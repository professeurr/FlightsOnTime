...
class FlightWeatherWrangling(flightWrangling: FlightWrangling, weatherWrangling: WeatherWrangling, weatherTimeFrame: Int, weatherTimeStep: Int) {
  ...
  def loadData(): DataFrame = {
    ...
    logger.info(s"OriginData.count() before missing weather ${OriginData.count()}")
    ...
    logger.info(s"OriginData.count() after missing weather ${OriginData.count()}")
    ...
    logger.info(s"DestinationData.count() before missing weather ${DestinationData.count()}")
    ...
    logger.info(DestinationData.schema.treeString)
    logger.info(s"DestinationData.count() after missing weather ${DestinationData.count()}")
